...
// Modified data type of irparams_struct.rawbuf[] from uint16_t to uint8_t
//static const uint16_t learnedNonModulatedToken = 0x0100U;
static const RAWBUF_DATA_TYPE learnedNonModulatedToken = LEARNEDNONMODULATEDTOKEN;
 ...
 */
void IRsend::sendPronto(const RAWBUF_DATA_TYPE *data, unsigned int length, uint_fast8_t aNumberOfRepeats) {
 ...
 */
static void compensateAndDumpSequence(Print *aSerial, const volatile RAWBUF_DATA_TYPE *data, size_t length, uint16_t timebase) {

    int iMICROS_PER_TICK = getMICROS_PER_TICK(); //Modified to get the correct iMICROS_PER_TICK
    for (uint_fast8_t i = 0; i < length; i++) {
        uint32_t tDuration = data[i] * iMICROS_PER_TICK;
        if (i & 1) {
            // Mark
...
static size_t compensateAndDumpSequence(String *aString, const volatile RAWBUF_DATA_TYPE *data, size_t length, uint16_t timebase) {
    size_t size = 0;

	//Modified for multiple IR
    int iMICROS_PER_TICK = getMICROS_PER_TICK();
    for (uint_fast8_t i = 0; i < length; i++) {
        uint32_t tDuration = data[i] * iMICROS_PER_TICK;
        if (i & 1) {
            // Mark
