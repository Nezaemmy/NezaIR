...
// data type was originally uint16_t modified to uint8_t
#define RAWBUF_DATA_TYPE uint8_t
#define LEARNEDNONMODULATEDTOKEN 0xFEU
//LEARNEDNONMODULATEDTOKEN is used in ir_pronto
//if you want to use uint16_t uncomment below
...
#endif
    RAWBUF_DATA_TYPE rawbuf[RAW_BUFFER_LENGTH]; ///< raw data / tick counts per mark/space, first entry is the length of the gap between previous and current command
};
...
// next 3 values are copies of irparams values - see IRremoteint.h
    RAWBUF_DATA_TYPE *rawbuf;           // deprecated, moved to decodedIRData.rawDataPtr->rawbuf ///< Raw intervals in 50uS ticks
...
class IRrecv {
public:
	struct irparams_struct irparams; // MultiIR_Mod: the irparams instance 
    IRrecv();
...
//Modified to get the correct MICROS_PER_TICK
